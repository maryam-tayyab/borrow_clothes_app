...
public class EditItemActivity extends AppCompatActivity implements Observer {
   ...
   private Switch status; // allows toggle between two states, You can add an OnCheckedChangeListener to respond to changes in the switch's state.
   ...
   @Override
   protected void onCreate(Bundle savedInstanceState) {//protected indicates that the method can be accessed from subclasses within the same package and by the class itself.
       super.onCreate(savedInstanceState);
       setContentView(R.layout.activity_edit_item);//R.layout.activity_edit_item is a way to reference a specific layout resource from your Android project, allowing you to set it as the content view of an activity.
       ...
       invisible.setVisibility(View.GONE); // View.GONE indicates the view is completely hidden and will not occupy any space in the layout.

       Intent intent = getIntent(); // Get intent from ItemsFragment Ti.e., the intent that started this activity.
       pos = intent.getIntExtra("position", 0);

       context = getApplicationContext(); // This method returns the context of the entire application, not just the current activity.
   ...
   public void saveItem(View view) {
       ...
       String title_str = title.getText().toString(); // same as using this.title.getText().toString(); this can sometimes be omitted when accessing class attributes in JAVA
       ...
       if (!status.isChecked()) { // // Handle when the switch off i.e., the item is not checked in but is borrowed
           String borrower_str = borrower_spinner.getSelectedItem().toString(); // .getSelectedItem(): This method retrieves the currently selected item from the Spinner as an object.
